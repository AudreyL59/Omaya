...
function BoolBadge({ v }: { v: boolean }) {
  return v ? (
    <Check className="w-3.5 h-3.5 text-c-brand inline-block" />
  ) : (
    <Minus className="w-3.5 h-3.5 text-c-ink-icon inline-block" />
  )
}
...
function colorClass(value: number, kind: ColorKind = 'none'): string {
  if (kind === 'none') return 'text-c-ink'
  switch (kind) {
    case 'racc':
      return value >= 70 ? 'text-c-brand'
        : value >= 60 ? 'text-amber-600' : 'text-red-600'
    case 'resil':
      return value <= 15 ? 'text-c-brand' : 'text-red-600'
    case 'consent':
      return value >= 80 ? 'text-c-brand' : 'text-red-600'
    case 'prise':
    case 'pc':
      return value >= 80 ? 'text-c-brand'
        : value >= 70 ? 'text-amber-600' : 'text-red-600'
    case 'dual':
      return value > 90 ? 'text-c-brand'
    ...
    case 'portab':
      return value >= 80 ? 'text-c-brand'
        : value >= 70 ? 'text-amber-600' : 'text-red-600'
    case 'mob':
      return value >= 49 ? 'text-c-brand'
        : value >= 30 ? 'text-amber-600' : 'text-red-600'
    case 'f200':
      return value >= 29 ? 'text-c-brand'
        : value >= 20 ? 'text-amber-600' : 'text-red-600'
    case 'dg':
      return value <= 15 ? 'text-c-brand' : 'text-red-600'
    case 'note':
      return value >= 9 ? 'text-c-brand'
...
function Kpi({
  ...
  return (
    <div className={`bg-white rounded-xl border border-c-line px-4 ${small ? 'py-2.5' : 'py-3'}`}>
      <div className="text-[11px] font-medium text-c-ink-faint uppercase tracking-wide truncate">
        {label}
      </div>
      <div className={`${small ? 'text-xl' : 'text-2xl'} font-bold tabular-nums mt-0.5 ${
        colored ? colorClass(numVal, tint) : 'text-c-ink'
      }`}>
        {typeof value === 'number'
          ? (Number.isInteger(value) ? value.toLocaleString('fr-FR') : value.toFixed(1))
          : value}
        {suffix && <span className="text-sm font-medium text-c-ink-faint ml-1">{suffix}</span>}
      </div>
      {sub && <div className="text-[11px] text-c-ink-faint-2 mt-0.5">{sub}</div>}
    </div>
  )
}
...
function DashboardSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <h3 className="text-sm font-semibold text-c-ink-soft uppercase tracking-wide mb-3">{title}</h3>
      {children}
    </div>
  )
}
...
const COLUMNS: ColDef[] = [
  { key: 'vendeur_nom', label: 'Vendeur', render: (r) => (
    <span>
      <span className="font-medium text-c-ink">{r.vendeur_nom}</span>{' '}
      <span className="text-c-ink-muted">{capitalize(r.vendeur_prenom)}</span>
  ...
  { key: 'partenaire', label: 'Part.', render: (r) => (
    <span className="font-semibold text-c-ink">{r.partenaire}</span>
  )},
  ...
  { key: 'poste', label: 'Poste', render: (r) => (
    <span className="text-xs text-c-ink-faint">{r.poste}</span>
  )},
  ...
  { key: 'client_mail', label: 'Mail', onlyIfDroit: 'InfoClientCoord',
    render: (r) => (
      <span className="text-xs text-c-ink-muted">{r.client_mail}</span>
    )},
  ...
  { key: 'info_partagee', label: 'Infos Contrats',
    render: (r) => (
      <span className="text-xs text-c-ink-muted line-clamp-2">{r.info_partagee}</span>
    )},
  ...
  { key: 'notation_info', label: 'Notation Info', onlyIfDroit: 'InfoNotation',
    render: (r) => <span className="text-xs text-c-ink-muted">{r.notation_info}</span> },
]
    ...
    <div className="p-6">
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          <Link
            to={listBase}
            className="p-2 text-c-ink-faint-2 hover:text-c-ink hover:bg-c-surface-medium rounded-lg"
          >
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <div className="min-w-0">
            <h1 className="text-xl font-bold text-c-ink truncate">
              {job?.titre || 'Extraction'}
            </h1>
            <p className="text-sm text-c-ink-faint mt-0.5">
      ...
      <div className="mt-5 border-b border-c-line flex items-center gap-1">
        <TabButton
  ...
  return (
    <button
      ...
      className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-colors ${
        active
          ? 'border-c-inverse text-c-ink'
          : 'border-transparent text-c-ink-faint hover:text-c-ink-soft'
      }`}
    >
      {icon}
      {label}
  ...
  return (
    <>
      <div className="flex flex-wrap items-center gap-3 mb-4">
        {partenairesUniques.length > 1 && (
          <select
            ...
            className="px-3 py-1.5 border border-c-line-strong rounded-lg text-sm"
          >
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-9 pr-3 py-1.5 border border-c-line-strong rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-c-line-strong w-48"
          />
        </div>
        <div className="relative">
          <Search className="w-4 h-4 text-c-ink-faint-2 absolute left-3 top-1/2 -translate-y-1/2" />
          <input
            ...
            className="pl-9 pr-3 py-1.5 border border-c-line-strong rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-c-line-strong w-40"
          />
        </div>
        <div className="relative">
          <Search className="w-4 h-4 text-c-ink-faint-2 absolute left-3 top-1/2 -translate-y-1/2" />
          <input
            ...
            className="pl-9 pr-3 py-1.5 border border-c-line-strong rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-c-line-strong w-36 font-mono"
          />
        </div>
        <div className="relative">
          <Search className="w-4 h-4 text-c-ink-faint-2 absolute left-3 top-1/2 -translate-y-1/2" />
      ...
      <div className="bg-white rounded-xl border border-c-line overflow-hidden">
              ...
              <tr>
                {visibleColumns.map((c) => (
                  <th
                    ...
                    className={`${c.align === 'right' ? 'text-right' : c.align === 'center' ? 'text-center' : 'text-left'} px-3 py-2.5 font-medium cursor-pointer select-none whitespace-nowrap hover:bg-c-surface-medium bg-c-surface-soft`}
                  >
        ...
        {data && data.total > 0 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-c-line bg-c-surface-soft text-sm">
            ...
            <div className="flex items-center gap-2">
              <select
                ...
                className="px-2 py-1 border border-c-line-strong rounded text-sm"
              >
              ...
              {!showAll && (
                <>
                  <button
                    ...
                    disabled={page <= 1}
                    className="px-2.5 py-1 border border-c-line-strong rounded disabled:opacity-40 hover:bg-white"
                  >
                    Précédent
                  </button>
                  <span className="tabular-nums text-c-ink-soft">{page} / {totalPages}</span>
                  <button
          ...
          <tbody className="divide-y divide-c-line-soft">
            {rows.map((r) => (
              <tr key={r.partenaire} className="hover:bg-c-surface-soft">
                ...
                <td className="px-3 py-2 text-right tabular-nums text-c-ink-muted">{num(r.envoye)}</td>
                ...
                <td className="px-3 py-2 text-right tabular-nums text-c-brand-strong font-medium">{num(r.payé)}</td>
                <td className="px-3 py-2 text-right tabular-nums text-c-ink-faint">{num(r.decomm)}</td>
                <td className="px-3 py-2 text-right tabular-nums text-red-500">{num(r.racc_activ_ko)}</td>
                <td className="px-3 py-2 text-right tabular-nums text-c-brand">{num(r.racc_active)}</td>
              </tr>
            ))}
            <tr className="bg-c-surface-soft font-semibold border-t-2 border-c-line">
              <td className="px-4 py-2 text-c-ink">TOTAL</td>
              <td className="px-3 py-2 text-right tabular-nums text-c-ink">{num(totals.brut)}</td>
              <td className="px-3 py-2 text-right tabular-nums text-c-ink-soft">{num(totals.temporaire)}</td>
              <td className="px-3 py-2 text-right tabular-nums text-c-ink-soft">{num(totals.envoye)}</td>
              ...
              <td className="px-3 py-2 text-right tabular-nums text-c-brand-strong">{num(totals.payé)}</td>
              <td className="px-3 py-2 text-right tabular-nums text-c-ink-muted">{num(totals.decomm)}</td>
              <td className="px-3 py-2 text-right tabular-nums text-red-600">{num(totals.racc_activ_ko)}</td>
              <td className="px-3 py-2 text-right tabular-nums text-c-brand-strong">{num(totals.racc_active)}</td>
            </tr>
          </tbody>
        </table>
      </div>
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((v) => (
                  <tr key={v.id_salarie} className="hover:bg-c-surface-soft">
                    ...
                    <td className="px-3 py-2 text-c-ink-faint text-xs">{v.poste || '—'}</td>
                    <td className="px-3 py-2 text-right tabular-nums font-semibold text-c-ink">
                      {v.nb_contrats.toLocaleString('fr-FR')}
                    </td>
                    <td className="px-3 py-2 text-right tabular-nums text-c-ink-soft">
                      {v.nb_hors_rejet.toLocaleString('fr-FR')}
                    </td>
                    <td className="px-3 py-2 text-right tabular-nums text-c-brand-strong font-medium">
                    ...
                    {partenaires.map((p) => (
                      <td key={p} className="px-3 py-2 text-right tabular-nums text-c-ink-faint">
...
function HorairesSignChart({ data }: { data: HoraireSignRow[] }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-c-line p-4">
      {/* Légende + total */}
      <div className="flex items-center gap-6 mb-3 text-xs">
        <div className="flex items-center gap-2">
          <span className="inline-block w-3 h-3 rounded-full bg-sky-500"></span>
          <span className="text-c-ink-soft">Ventes finalisées</span>
          <span className="text-c-ink-faint-2 tabular-nums">({totalVentes})</span>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        {hover !== null && (
          <div
            className="absolute bg-white border border-c-line rounded-lg shadow-md px-3 py-2 text-xs pointer-events-none"
            style={{
              left: `${(x(hover) / W) * 100}%`,
              top: 4,
          ...
          >
            <div className="font-semibold text-c-ink mb-1">{filled[hover].h}h</div>
            <div className="flex items-center gap-2">
              <span className="inline-block w-2 h-2 rounded-full bg-sky-500"></span>
              <span className="text-c-ink-faint">Ventes :</span>
              <span className="tabular-nums text-c-ink">{filled[hover].ventes}</span>
            </div>
          </div>
        )}
      </div>
