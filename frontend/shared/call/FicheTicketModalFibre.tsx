...
interface Props {
  ...
  // `base` = prefixe ressource (ex: /api/call/fibre/tickets). Toutes les URLs
  // sont construites en `${base}/...`, SAUF le GET fiche (structure differente
  // cote Vendeur) fourni par `ficheUrl`.
  base: string
  ficheUrl: (id: string) => string
}
...
export default function FicheTicketModal({ idTicket, onClose, onAfterAction, readonly = false, base, ficheUrl }: Props) {
  ...
  useEffect(() => {
    ...
    if (!isPortabilite && idTicket) {
      fetch(`${base}/${idTicket}/panier/${selectedPanierId}/lettre-resil`, {
        headers: { Authorization: `Bearer ${getToken()}` },
      })
