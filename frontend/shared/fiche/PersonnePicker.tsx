/**
 * PersonnePicker partage : popup de recherche/selection d'un salarie.
 *
 * Version shared du composant ADM (frontend/adm/src/components/PersonnePicker.tsx),
 * paramétrée avec apiBase + getToken pour pouvoir etre utilisee depuis :
 *   - frontend/adm (fiche salarie ADM)
 *   - frontend/shared/tickets (tickets sortie RH)
 *
 * L'endpoint cible est `${apiBase}/salaries/search?q=<query>` qui doit
 * exister cote intranet appelant (cf. /api/adm/salaries/search).
 */
...
export default function PersonnePicker({
  ...
  const doSearch = () => {
    ...
    setLoading(true)
    fetch(
      `${apiBase}/salaries/search?q=${encodeURIComponent(search.trim())}`,
