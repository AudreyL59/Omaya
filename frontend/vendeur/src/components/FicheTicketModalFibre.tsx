/**
 * ============================================================================
 * SYNCED FROM frontend/fibre/src/components/FicheTicketModal.tsx
 *
 ...
 *  - Chemins d'endpoints `/tickets/{id}/...` remap en `/fiche-fibre/{id}/...`
 *
 * NB : au 2026-07-20, seul l'endpoint GET /fiche-fibre/{id} est porte cote
 * Vendeur (etape 4b-1). Les endpoints d'ecriture (save-vente, save-offre,
 * verrou, actions, documents, lettre-resil) restent a porter (etape 4b-5+).
 * Ils echoueront en 404 tant que le portage n'est pas fait.
 * ============================================================================
 *
...
export default function FicheTicketModal({ idTicket, onClose, onAfterAction }: Props) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const r = await fetch(`${API_BASE}/fiche-fibre/${idTicket}`, {
        ...
        fetch(`${API_BASE}/fiche-fibre/${idTicket}/documents?client_pro=${d.client.client_pro ? 1 : 0}`, {
          headers: { Authorization: `Bearer ${getToken()}` },
        })
  ...
  useEffect(() => {
    ...
    if (!isPortabilite && idTicket) {
      fetch(`${API_BASE}/fiche-fibre/${idTicket}/panier/${selectedPanierId}/lettre-resil`, {
        headers: { Authorization: `Bearer ${getToken()}` },
      })
  ...
  const handleSaveVente = async () => {
    ...
    try {
      ...
      const r = await fetch(`${API_BASE}/fiche-fibre/${idTicket}/save-vente`, {
        method: 'POST',
        headers: {
  ...
  const handleSaveOffre = async () => {
    ...
    try {
      ...
      const r = await fetch(`${API_BASE}/fiche-fibre/panier/${offre.id}/save-offre`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${getToken()}`,
          'Content-Type': 'application/json',
  ...
  const handlePrendreAppel = async (force: boolean = false) => {
    ...
    try {
      const r = await fetch(`${API_BASE}/fiche-fibre/${idTicket}/verrou/prendre`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${getToken()}`,
          'Content-Type': 'application/json',
  ...
  const handleLacherAppel = async () => {
    ...
    setVerrouLoading(true)
    try {
      const r = await fetch(`${API_BASE}/fiche-fibre/${idTicket}/verrou/lacher`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${getToken()}` },
      })
      if (!r.ok) {
  ...
  const handleAnnulerLigne = async (motifs: string[], precisions: string) => {
    ...
    try {
      const r = await fetch(`${API_BASE}/fiche-fibre/panier/${selectedPanierId}/annuler-ligne`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${getToken()}`,
          'Content-Type': 'application/json',
        ...
        actionDialog === 'valider'
          ...
          : actionDialog === 'annulVente'
            ...
            : actionDialog === 'renvoiResil'
              ...
              : 'renvoyer-complement'
      const r = await fetch(`${API_BASE}/fiche-fibre/${idTicket}/${path}`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${getToken()}`,
          'Content-Type': 'application/json',
