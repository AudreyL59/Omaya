/**
 * ============================================================================
 * SYNCED FROM frontend/energie/src/components/FicheTicketModal.tsx
 *
 * Copie utilisee par la page /vendeur/tickets-call (Suivi Ticket Call).
 * TOUTE modif d'un des 4 exemplaires FicheTicketModal (Fibre / Energie
 * source + Fibre / Energie Vendeur) doit etre repercutee dans les autres.
 * Cf. memoire feedback_fiche_ticket_modal_sync.
 *
 * Differences avec la source :
 *  - API_BASE pointe sur /api/vendeur/tickets-call/suivi
 *  - Chemins d'endpoints `/tickets/{id}/...` remap en `/fiche-energie/{id}/...`
 *
 * NB : au 2026-07-20, seul l'endpoint GET /fiche-energie/{id} est porte
 * cote Vendeur (etape 4b-1). Les endpoints d'ecriture (save-vente,
 * save-offre, verrou, actions, documents, clarification) restent a porter
 * (etape 4b-5+). Ils echoueront en 404 tant que le portage n'est pas fait.
 * ============================================================================
 *
...
export default function FicheTicketModal({ idTicket, onClose, onAfterAction }: Props) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const r = await fetch(`${API_BASE}/fiche-energie/${idTicket}`, {
        ...
        fetch(`${API_BASE}/fiche-energie/${idTicket}/documents?client_pro=${d.client.client_pro ? 1 : 0}`, {
          headers: { Authorization: `Bearer ${getToken()}` },
        })
  ...
  useEffect(() => {
    ...
    let cancelled = false
    fetch(`${API_BASE}/fiche-energie/panier/${selectedPanierId}/clarification`, {
      headers: { Authorization: `Bearer ${getToken()}` },
    })
  ...
  const handlePrendreAppel = async (force = false) => {
    ...
    try {
      const r = await fetch(`${API_BASE}/fiche-energie/${idTicket}/verrou/prendre`, {
        method: 'POST',
  ...
  const handleLacherAppel = async () => {
    ...
    setVerrouLoading(true)
    try {
      const r = await fetch(`${API_BASE}/fiche-energie/${idTicket}/verrou/lacher`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${getToken()}` },
      })
      if (!r.ok) {
  ...
  const handleAnnulerLigne = async (motifs: string[], precisions: string) => {
    ...
    try {
      const r = await fetch(`${API_BASE}/fiche-energie/panier/${selectedPanierId}/annuler-ligne`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${getToken()}`, 'Content-Type': 'application/json' },
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      const path =
        ...
        : 'renvoyer-complement'
      const r = await fetch(`${API_BASE}/fiche-energie/${idTicket}/${path}`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${getToken()}`, 'Content-Type': 'application/json' },
  ...
  const handleSaveClient = async () => {
    ...
    try {
      ...
      const r = await fetch(`${API_BASE}/fiche-energie/${idTicket}/save-vente`, {
        method: 'POST',
        headers: {
  ...
  const handleSaveOffre = async (offre: FicheOffre, silent = false) => {
    ...
    try {
      ...
      const r = await fetch(`${API_BASE}/fiche-energie/panier/${offre.id}/save-offre`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${getToken()}`,
          'Content-Type': 'application/json',
