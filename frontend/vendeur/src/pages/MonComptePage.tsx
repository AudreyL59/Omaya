...
import { User, MapPin, FileText, Loader2, FileDown, X, ExternalLink } from 'lucide-react'
...
export default function MonComptePage() {
  ...
  const { identite: id, coordonnees: co } = data

  return (
    <div className="p-8 max-w-4xl">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-bold text-gray-900">Mon Compte</h1>
        <p className="text-gray-500 mt-1">Vos informations personnelles</p>
      </motion.div>
      ...
      <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6 flex gap-8">
              ...
              <div className="grid grid-cols-2 gap-x-12 gap-y-4 flex-1">
                <Field label="Civilité" value={CIVILITE[id.civilite] || ''} />
                <Field label="Sexe" value={id.sexe === 'F' ? 'Féminin' : id.sexe === 'M' ? 'Masculin' : id.sexe} />
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6 mt-4">
              <h3 className="text-sm font-semibold text-gray-700 mb-4">Naissance</h3>
            ...
            {/* Administratif */}
            <div className="bg-white rounded-xl border border-gray-200 p-6 mt-4">
              <h3 className="text-sm font-semibold text-gray-700 mb-4">Informations administratives</h3>
              <div className="grid grid-cols-3 gap-x-12 gap-y-4">
                <Field label="N° Sécurité Sociale" value={id.num_ss} />
                <Field label="CPAM" value={id.cpam} />
                <Field label="N° CIN" value={id.num_cin} />
                <Field label="Situation familiale" value={SITUATION_FAM[id.situation_fam] || ''} />
                <Field label="Avec enfant" value={id.avec_enfant} />
                <Field label="Nombre d'enfants" value={id.nb_enfants} />
                <Field label="Travailleur handicapé" value={id.travailleur_handi} />
              </div>
            </div>
          </motion.div>
        )}
        ...
        {activeTab === 'coordonnees' && (
          ...
          >
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-4">Coordonnées postales et téléphoniques</h3>
              <div className="grid grid-cols-2 gap-x-12 gap-y-4">
                ...
                <div className="flex gap-4">
                  <div className="w-24"><Field label="Code Postal" value={co.cp} /></div>
                  <div className="flex-1"><Field label="Ville" value={co.ville} /></div>
                </div>
                <Field label="Courriel" value={co.mail} />
              </div>
            </div>

            {/* Contact urgence + bancaire */}
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div className="bg-white rounded-xl border border-gray-200 p-6">
                <h3 className="text-sm font-semibold text-gray-700 mb-4">Personne à contacter en cas d'urgence</h3>
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-6">
                <h3 className="text-sm font-semibold text-gray-700 mb-4">Coordonnées bancaires</h3>
                <div className="space-y-4">
        ...
        {activeTab === 'documents' && (
          ...
          >
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-4">Fiches de salaire</h3>
              {docsLoading ? (
                <div className="flex items-center justify-center py-12">
                  <Loader2 className="w-6 h-6 text-gray-300 animate-spin" />
                </div>
              ) : documents.length === 0 ? (
                <p className="text-gray-400 text-sm py-8 text-center">Aucun document</p>
              ) : (
                <>
                  <p className="text-xs text-gray-400 mb-2">{documents.length} document{documents.length > 1 ? 's' : ''}</p>
                    ...
                    <table className="w-full text-sm">
                      <thead className="sticky top-0 bg-gray-50 z-10">
                        <tr className="border-b border-gray-200">
                          ...
                          <th className="text-right py-2 px-3 text-gray-500 font-medium w-24">Taille</th>
                          <th className="text-right py-2 px-3 text-gray-500 font-medium w-32">Date</th>
                        </tr>
                      </thead>
                      <tbody>
                        {documents.map((doc, i) => (
                          ...
                          >
                            <td className="py-2.5 px-3">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-gray-700 truncate">{doc.nom}</span>
                              </div>
                            </td>
                            <td className="py-2.5 px-3 text-right text-gray-500">{doc.taille_mo} Mo</td>
                            <td className="py-2.5 px-3 text-right text-gray-500">{doc.date}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <h3 className="text-sm font-semibold text-gray-700 truncate">{previewDoc.nom}</h3>
                </div>
