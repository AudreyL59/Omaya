/**
 * Picker de recruteur/salarié (transposition Fen_RechercheNomSalarié WinDev).
 *
 * Recherche par nom via /api/adm/agenda-recrutement/recruteurs?q=...
 * (endpoint qui retourne les salaries accessibles a l'utilisateur connecte).
 *
 * Utilise par :
 *  - AgendaRecrutementPage (selection du recruteur a charger)
 *  - AgendaDetailModal (bouton "Choisir l'Operateur" de Fen_AgendaDetail)
 */
...
interface Props {
  ...
  /** Titre de la modale (par defaut : "Choisir le recruteur"). */
  title?: string
}

export default function RecruteurPicker({
  onClose,
  onSelect,
  title = 'Choisir le recruteur',
}: Props) {
