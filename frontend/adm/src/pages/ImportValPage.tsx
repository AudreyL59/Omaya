/**
 * Fen_ImportVAL — Import partenaire VALANDRE.
 *
...
export default function ImportValPage() {
  useDocumentTitle('Import VALANDRE')
  ...
  useEffect(() => {
    fetch(`${API_BASE}/imports/partenaires`, {
      ...
      .then((rows: Array<{ prefixe_bdd: string; logo_b64: string }>) => {
        const iag = (rows || []).find(p => p.prefixe_bdd === 'VAL')
  ...
  const demarrer = async () => {
    ...
    try {
      ...
      const r = await fetch(`${API_BASE}/imports/val/run`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${getToken()}` },
        body: fd,
      })
  ...
  return (
    <div className="p-4 flex flex-col h-[calc(100vh-120px)]"
      ...
      <PageHeader
        iconNode={
          logoSrc ? <img src={logoSrc} alt="VALANDRE" className="h-7" /> : undefined
        }
        title="Import VALANDRE"
