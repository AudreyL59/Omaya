/**
 * Fen_ImportSFR — Import partenaire SFR (10 types).
 *
...
const TYPES = [
  { id: 1, label: 'Base Journalière Fibre' },
  { id: 2, label: 'Base Journalière Mobile' },
  { id: 3, label: 'Base Journalière CALL' },
  { id: 4, label: 'Base Hebdo' },
  { id: 5, label: 'Import Options' },
  { id: 6, label: 'RUN' },
  { id: 7, label: 'Call RET - KO' },
  { id: 8, label: 'Call RET - Racc' },
  { id: 9, label: 'Call RET - Vente ADD' },
  { id: 10, label: 'Call RET - RDV Tech' },
]

interface Resume {
  nb_fichiers: number; nb_ajoutes: number; nb_modifies: number
  nb_modif_vend: number; nb_migrations: number; nb_non_modifies: number
  nb_erreurs: number; nb_introuvables: number; nb_doublons: number
...
export default function ImportSfrPage() {
  useDocumentTitle('Import SFR')
  ...
  useEffect(() => {
    fetch(`${API_BASE}/imports/partenaires`, {
      ...
      .then((rows: Array<{ prefixe_bdd: string; logo_b64: string }>) => {
        const iag = (rows || []).find(p => p.prefixe_bdd === 'SFR')
  ...
  const demarrer = async () => {
    ...
    try {
      ...
      const r = await fetch(`${API_BASE}/imports/sfr/run`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${getToken()}` },
        body: fd,
      })
  ...
  return (
    <div className="p-4 flex flex-col h-[calc(100vh-120px)]"
      ...
      <PageHeader
        iconNode={
          logoSrc ? <img src={logoSrc} alt="SFR" className="h-7" /> : undefined
        }
        title="Import SFR"
