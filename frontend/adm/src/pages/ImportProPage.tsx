/**
 * Fen_ImportPRO — Import partenaire PROTECTED (assurance appareils).
 *
...
export default function ImportProPage() {
  useDocumentTitle('Import PROTECTED')
  ...
  useEffect(() => {
    fetch(`${API_BASE}/imports/partenaires`, {
      ...
      .then((rows: Array<{ prefixe_bdd: string; logo_b64: string }>) => {
        const iag = (rows || []).find(p => p.prefixe_bdd === 'PRO')
  ...
  const demarrer = async () => {
    ...
    try {
      ...
      const r = await fetch(`${API_BASE}/imports/pro/run`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${getToken()}` },
        body: fd,
      })
  ...
  return (
    <div className="p-4 flex flex-col h-[calc(100vh-120px)]"
      ...
      <PageHeader
        iconNode={
          logoSrc ? (
            <img src={logoSrc} alt="PROTECTED" className="h-7" />
          ) : undefined
        }
        title="Import PROTECTED"
