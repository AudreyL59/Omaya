/**
 * Fen_ImportSTR — Import partenaire STRATO.
 *
...
export default function ImportStrPage() {
  useDocumentTitle('Import STRATO')
  ...
  useEffect(() => {
    fetch(`${API_BASE}/imports/partenaires`, {
      ...
      .then((rows: Array<{ prefixe_bdd: string; logo_b64: string }>) => {
        const iag = (rows || []).find(p => p.prefixe_bdd === 'STR')
  ...
  const demarrer = async () => {
    ...
    try {
      ...
      const r = await fetch(`${API_BASE}/imports/str/run`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${getToken()}` },
        body: fd,
      })
  ...
  return (
    <div className="p-4 flex flex-col h-[calc(100vh-120px)]"
      ...
      <PageHeader
        iconNode={
          logoSrc ? <img src={logoSrc} alt="STRATO" className="h-7" /> : undefined
        }
        title="Import STRATO"
