"""Service Fen_ImportVAL (ADM Imports Bases -> VALANDRE).
...
class ImportValParams(BaseModel):
...
class ImportValResume(BaseModel):
    nb_fichiers: int = 0
    nb_ajoutes: int = 0
    nb_valides: int = 0
    nb_resilies: int = 0
...
class ImportValResult(BaseModel):
    ok: bool
    type_import: int
    type_label: str
    simulation: bool
    resume: ImportValResume
...
def _id_produit_val_by_lib(lib_produit: str) -> tuple[int, str]:
    """Lookup pgt_val_produit by lib_produit LIKE."""
    if not lib_produit:
        return (0, "")
    db = get_pg_connection("adv")
    r = db.query_one(
        """SELECT id_produit, lib_produit FROM adv.pgt_val_produit
...
def _id_etat_val_by_lib(lib_statut: str) -> int:
    """Lookup pgt_val_etat_contrat by lib_etat LIKE."""
    if not lib_statut:
        return 0
    db = get_pg_connection("adv")
    r = db.query_one(
        """SELECT id_etat FROM adv.pgt_val_etat_contrat
...
def _lookup_vendeur_val_nom_prenom(nom: str, prenom: str) -> int:
...
def _lookup_tk_call_val(num_bs: str) -> dict:
...
def _info_salarie_val(id_sal: int) -> dict:
    if not id_sal:
        return {}
    db = get_pg_connection("rh")
    r = db.query_one(
...
def _ajoute_histo_val_etat(id_contrat: int, old_etat: int, new_etat: int,
                            date_paiement: str, op_id: int) -> None:
    if not id_contrat:
        return
    db = get_pg_connection("adv")
    auto = db.query_one(
        "SELECT COALESCE(MAX(id_histo_auto), 0) + 1 AS n FROM adv.pgt_val_histo_etat_ctt"
    )
    db.query(
        """INSERT INTO adv.pgt_val_histo_etat_ctt
...
def _create_val_contrat(td: dict, op_id: int) -> int:
    ...
    id_contrat = _new_id()
    auto = db.query_one(
        "SELECT COALESCE(MAX(id_contrat_auto), 0) + 1 AS n FROM adv.pgt_val_contrat"
    )
...
def _import_journalier_val(
    ...
    - Si BS n'existe pas :
      ...
      * Ajout (sheet 1, creation contrat en prod via _create_val_contrat)
    """
    ...
    prod_info = db.query_one(
        "SELECT lib_produit FROM adv.pgt_val_produit WHERE id_produit = ? LIMIT 1",
        (ID_PROD_VAL,),
    )
    ...
    for i in range(2, (ws.max_row or 0) + 1):
        ...
        # Determine id_etat
        id_etat = _id_etat_val_journ(statut_vente, remarques)
        ...
        ctt = db.query_one(
            """SELECT id_contrat, id_salarie, id_client, num_bs,
                      date_signature, id_etat_contrat, format_numerique
        ...
        if ctt:
            ...
            etat_db_info = db.query_one(
                """SELECT id_type_etat FROM adv.pgt_val_etat_contrat
            ...
            info_sal_db = _info_salarie_val(id_sal_db)
            agence, equipe, _ = _affectation_val(id_sal_db)
            changement_etat = "non"
            ...
            if id_etat != etat_db and type_etat_db in (1, 2):
                ...
                if not p.simulation:
                    try:
                        ...
                        _ajoute_histo_val_etat(id_contrat, etat_db, id_etat,
                                               "", op_id)
                    except Exception as e:
            ...
            modifies.append({
                ...
                "FormatNumerique": bool(ctt.get("format_numerique")),
        ...
        if id_vendeur == 0:
            ...
            info_sal = _info_salarie_val(id_vendeur)
...
def _affectation_val(id_salarie: int) -> tuple[str, str, bool]:
...
def _import_run_val(
    ...
    try:
        ...
        return
    ws = wb.active
    cols = {k: _col_letter_to_index(v) for k, v in COLS_RUN_VAL.items()}
    db = get_pg_connection("adv")
    ...
    if not p.simulation:
        for row in runs:
            ...
            try:
                ...
                _ajoute_histo_val_etat(int(id_ct), int(old_etat), int(new_etat),
...
def _import_resil_hebdo_val(
    ...
    try:
        ...
        return
    ws = wb.active
    cols = {k: _col_letter_to_index(v) for k, v in COLS_RH_VAL.items()}
    db = get_pg_connection("adv")
...
def run_import_val(
    p: ImportValParams, files: list[tuple[str, bytes]], op_id: int,
) -> ImportValResult:
    """Dispatcher principal."""
    label = TYPE_LABELS.get(p.type_import, "?")
    if not files:
        return ImportValResult(
            ok=False, type_import=p.type_import, type_label=label,
            simulation=p.simulation, resume=ImportValResume(),
            message="Aucun fichier fourni.",
        )

    resume = ImportValResume(nb_fichiers=len(files))
    ...
    if not p.simulation:
        ...
        for row in ajoutes:
            ...
            if not pl or not pl.get("id_salarie"):
                continue
            try:
                new_id = _create_val_contrat(pl, op_id)
    ...
    _attach_xlsx_and_mail_val(res, op_id)
    return res


def _build_xlsx_val(res: ImportValResult) -> bytes:
...
def _attach_xlsx_and_mail_val(res: ImportValResult, op_id: int) -> None:
    ...
    sujet = (f"{sujet_pref}Importation {res.type_label} VALANDRE "
